        ...
        if self.gc.databasePath is not None:
            with contextlib.closing(sqlite3.connect(self.gc.databasePath)) as dbcon:
                if self.param == 'lte_physical_cell_id_1':
                    sqlstr_for_get_pci = "select log_hash, time, {}, geom from lte_cell_meas order by log_hash, time".format(self.param)
                else:
                    sqlstr_for_get_pci = "select log_hash, time, {}, geom, lte_physical_cell_id_1 from lte_cell_meas order by log_hash, time".format(self.param)
                lte_cell_meas_df = pd.read_sql(sqlstr_for_get_pci, dbcon)
