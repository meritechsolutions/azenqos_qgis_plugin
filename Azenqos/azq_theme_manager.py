        ...
        try:
            ...
            try:
                ...
                if len(matched_rows) == 0:
                    raise Exception(
                        "failed to find matching row in azq_global elm csv list"
                    )
                dprint(
                    "matched_rows.iloc[0].n_arg_max:", matched_rows.iloc[0].n_arg_max
                )
...
def get_theme_df_for_column(
    ...
    if theme_xml_file.endswith(".csv"):  # not support reverse_cum
        ...
        try:
            theme_df = get_csv_theme_df_for_csv_file(theme_xml_file)
            param_col = preprocess_azm.get_elm_name_from_param_col_with_arg(
                param_col_with_arg
            )
            ...
            theme_for_param_col["color_rgb_hex"] = theme_for_param_col[
                "color_rgb_hex"
            ].astype(str)
            ...
            theme_for_param_col.ColorXml = "#" + (
                theme_for_param_col.ColorXml.astype(str).replace("#", "")
            )
            ...
            if theme_for_param_col is None or len(theme_for_param_col) == 0:
                raise Exception(
                    "csv theme_for_param_col ret len 0 - try gen from data instead..."
                )

            print("csv theme:", theme_for_param_col)
            if pd.isnan(theme_for_param_col.Lower).all():
                raise Exception(
                    "csv theme all empty - raise exception to gen from data instead..."
                )
            return theme_for_param_col
        except Exception as cte:
            print(
                "WARNING: get_theme_df_for_column [{}] - csv theme mode get exception: {}".format(
                    param_col, str(cte)
                )
            )
    ...
    try:
        ...
        if theme_xml_file.endswith(".csv"):
            raise Exception(
                "theme file is csv and got 0 len case fallback to gen from data instead..."
            )
        ...
        for elm in root.iter("ReportGeneratorSetting"):
            ...
            if eid.text == param_col:
                ...
                try:
                    is_reverse_cum = (
                        elm.find("isReverseComulative").text.strip() == "true"
                    )
                except Exception as e:
                ...
                if retdfempty and theme_xml_file != g_ori_default_theme_file:
                    print(
                        "retdf.size 0 in non-default theme try get from default_theme.xml: theme_xml_file {} g_ori_default_theme_file {}".format(
                            theme_xml_file, g_ori_default_theme_file
                        )
                    )
                    retdf = get_theme_df_for_column(
                        ...
                        override_xml_file=g_ori_default_theme_file,
                    )

                if retdf is None:
                    raise Exception(
                        "retdf == None - fall back to generate theme from data insted..."
                    )

                if retdf.size == 0:
                    raise Exception(
                        "No ranges for this element - fall back to generate theme from data insted..."
                    )
        ...
        if found_elm == False and theme_xml_file != g_ori_default_theme_file:
            ...
            if retdf is None:
                raise Exception(
                    "case found_elm false retdf == None - fall back to generate theme from data insted..."
                )

            if retdf.size == 0:
                raise Exception(
                    "case found_elm false No ranges for this element - fall back to generate theme from data insted..."
                )
        ...
        print(
            "WARNING: parse theme file: {} param_col: {} exception: {}".format(
                theme_xml_file, param_col, str(e)
            )
        )
    ...
    if param_col_with_arg == "wcdma_aset_ecio_1":
        print(
            "azq_theme_manager failed to get theme for wcdma_aset_ecio_1 - try get from xml/csv with name wcdma_ecio_1 instead..."
        )
        return get_theme_df_for_column(
            "wcdma_ecio_1",
            data_df=None,
            dbcon=None,
            table_name=None,
    ...
    print(
        "Get theme from xml file failed - falling back to generate_theme_from_data..."
    )
    ...
    col_is_id = is_param_col_an_id(
        param_col, table_name=table_name
    )  # like pci, psc, bcch arfcn

    param_col_is_str = False
    try:
        if pd.api.types.is_string_dtype(data_df[param_col]):
            print(
                "azq_theme_manager: param_col is_string_dtype so set is_param_col_an_id"
            )
    ...
    gdret = generate_theme_from_data(
        data_df, param_col, all_unique_vals_per_theme=col_is_id
    )
...
def generate_theme_from_data(df, param_col, all_unique_vals_per_theme=False):
    ...
    if all_unique_vals_per_theme:
        print(
            "get unique vals for param_col:",
            param_col,
            "azq_cell_file.CELL_FILE_RATS:",
            azq_cell_file.CELL_FILE_RATS,
        )
        ...
        if param_col not in df.columns:
            raise Exception(
                "ABORT param_col {} not in supplied df columns".format(param_col)
            )
                ...
                if isinstance(param_col, str):
                    if uv is not None:
                        if param_col == "rat":
                            ...
                            or param_col.startswith("wcdma_sc")
                            or param_col.startswith("gsm_arfcn_bcch")
                        ):
                            try:
                    ...
                    print(
                        "WANRING: param_col is not str - not doing override checks..."
                    )
                #####
...
def generate_theme_with_step(start, end, step):
    if None in [start, end, step]:
        raise Exception(
            "generate_theme_with_step Exception start, end, step is invalid:",
            start,
            end,
            step,
        )
