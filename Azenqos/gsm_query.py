...
def get_gsm_radio_params_disp_df(dbcon, time_before):
    ...
    df = params_disp_df.get(dbcon, parameter_to_columns_list, time_before, not_null_first_col=False, custom_lookback_dur_millis=gc.DEFAULT_LOOKBACK_DUR_MILLIS)
...
def get_gsm_serv_and_neigh__df(dbcon, time_before):
    ...
    cell_col_prefix_renamed = ["Cell Name", "BSIC", "ARFCN ", "RxLev", "C1", "C2", "C31", "C32"]
    serv_col_prefix_sr = pd.Series(["gsm_cellfile_matched_cellname", "gsm_bsic",  "gsm_arfcn_bcch", "gsm_rxlev_sub_dbm", "gsm_c1", "gsm_c2", "gsm_c31", "gsm_c32"])
    parameter_to_columns_list = [
        (
            "serv",
    ...
    df = params_disp_df.get(dbcon, parameter_to_columns_list, time_before, default_table="gsm_cell_meas", not_null_first_col=False, custom_lookback_dur_millis=gc.DEFAULT_LOOKBACK_DUR_MILLIS)
    ...
    df2 = params_disp_df.get(dbcon, parameter_to_columns_list, time_before, default_table="gsm_serv_cell_info", not_null_first_col=False, custom_lookback_dur_millis=gc.DEFAULT_LOOKBACK_DUR_MILLIS)
    df2.columns = ["CellGroup", "LAC"]
    df.insert(2,"LAC", df2["LAC"])

    neigh_col_prefix_sr = pd.Series(["gsm_cellfile_matched_neighbor_cellname", "gsm_cellfile_matched_neighbor_lac_", "gsm_neighbor_bsic_", "gsm_neighbor_arfcn_", "gsm_neighbor_rxlev_dbm_", "gsm_neighbor_c1_", "gsm_neighbor_c2_", "gsm_neighbor_c31_", "gsm_neighbor_c32_"])
    neigh_n_param = 32
    ...
    df = params_disp_df.get(dbcon, parameter_to_columns_list, time_before, default_table="gsm_cell_meas", not_null_first_col=False, custom_lookback_dur_millis=gc.DEFAULT_LOOKBACK_DUR_MILLIS)
    df.columns = ["CellGroup", "Cell Name", "LAC", "BSIC", "ARFCN ", "RxLev", "C1", "C2", "C31", "C32"]
    df_list.append(df)
...
def get_gsm_current_channel_disp_df(dbcon, time_before):
    ...
    parameter_to_columns_list = [          
        ...
        (  
            ["Mobile Allocation Index Offset (MAIO)", "Hopping Sequence Number (HSN)"], ["gsm_maio", "gsm_hsn"], "gsm_rr_chan_desc"
        ),  
        (  
        ...
        (  
            ["Speech Codec TX", "Speech Codec RX"], ["gsm_speechcodectx", "gsm_speechcodecrx"], "vocoder_info"
        ),    
        (  
...
def get_coi_df(dbcon, time_before):
    ...
    df = params_disp_df.get(dbcon, parameter_to_columns_list, time_before, default_table="gsm_coi_per_chan", not_null_first_col=True, custom_lookback_dur_millis=gc.DEFAULT_LOOKBACK_DUR_MILLIS)
    df.columns = ["CellGroup"]+cell_col_prefix_renamed
    df_list.append(df)
    ...
    avg = sum(map(lambda y: list(map(lambda x: x+"{}".format(y+1), avg_col_prefix_sr)),range(avg_n_param)),[])
    parameter_to_columns_list = [
        (
            "Avg",
            ["\"\""]
