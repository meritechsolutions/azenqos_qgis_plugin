...
class CellLayerTask(QgsTask):
    ...
    def create_cell_layer(self, df, system, name, color):
        features = df[df['system'].str.lower() == system].apply(
            cell_to_polygon, axis=1).values.tolist()
    ...
    def run(self):
        try:
            df = read_cell_file(self.path)
            nr_cells_layer = self.create_cell_layer(df, "nr", "5G cells", get_default_color_for_index(0))
            lte_cells_layer = self.create_cell_layer(df, "lte", "4G cells", get_default_color_for_index(1))
            wcdma_cells_layer = self.create_cell_layer(df, "wcdma", "3G cells", get_default_color_for_index(2))
            gsm_cells_layer = self.create_cell_layer(df, "gsm", "2G cells", get_default_color_for_index(3))
            self.cells_layers = [nr_cells_layer, lte_cells_layer, wcdma_cells_layer, gsm_cells_layer]
            azq_utils.write_local_file("config_prev_cell_file", self.path)
        except:
