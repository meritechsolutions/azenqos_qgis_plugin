    ...
    for rat in CELL_FILE_RATS:
        try:
            ...
            for param in params:
                ...
                if theme is None:
                    print("prepopulate_cell_themes: got theme for param:", param, "got None theme so skip")
                    continue
                ...
                try:
                    ...
                    pass
                print("prepopulate_cell_themes: got theme for param:", param)  #, "theme:", theme
                set_theme(rat, theme)
                print("prepopulate_cell_themes_for_rats_that_have_cellfiles: set_theme done for param:", param)
        except Exception:
            type_, value_, traceback_ = sys.exc_info()
            exstr = str(traceback.format_exception(type_, value_, traceback_))
            print("WARNING: exception in prepopulate_cell_themes_for_rats_that_have_cellfiles(dbcon):" + exstr)
...
def set_cell_fp(rat, cfp, raise_exception_if_check_failed=False):
    ...
    print("set_cell_fp: raise_exception_if_check_failed: {}".format(raise_exception_if_check_failed))

    # test parse if cellfile is valid or not - raise exception here
    get_cell_file_df(cfp, [g_main_cell_col[rat]], raise_exception_if_check_failed=raise_exception_if_check_failed)
...
def set_theme(rat, df):
    ...
    if g_theme_dict[rat] is None:
        ...
        try:
            ...
            print("WARNING: azq_cell_file.set_theme convert match_value to numer failed exception:", ex)
...
def calculate_point_with_degree_from_y_axis(x_origin, y_origin, degree, cell_arm_length=70):
    factor = 200
    if 90 < degree <= 270:
        factor = factor*-1
        utils.warn("factor * -1")
    ...
    return resize_line_length(x_origin,y_origin,x_target,y_target, new_length=cell_arm_length)
...
def plot_cell(ax, x, y, start_degree, degree, color='grey', cell_label='', cell_arm_length=70, used=False, site_label='',
              cellfile_layer=plot_param_zorders.ZORDER_PLOT_CELL, site_label_offset=0, site_font_size=None, cell_font_size=None):
    ...
    if start_degree < 0:
        start_degree += 360
    xy_pos_2 = calculate_point_with_degree_from_y_axis(x, y, start_degree, cell_arm_length=cell_arm_length)
    xy_pos_3 = calculate_point_with_degree_from_y_axis(x, y, float(start_degree) + float(degree), cell_arm_length=cell_arm_length)
    verts = [
        (x, y),  # left, bottom
        xy_pos_2,
        xy_pos_3,
    ...
    if 0 <= start_degree <= 90:
        ...
        ax.annotate(text, xy=((xy_pos_2[0]+xy_pos_3[0])/2 - 15, (xy_pos_2[1]+xy_pos_3[1])/2 - 15))
    """
    ax.annotate(cell_label, xy=((xy_pos_2[0] + xy_pos_3[0]) / 2 + 0, (xy_pos_2[1] + xy_pos_3[1]) / 2 - 0), alpha=alpha, zorder=plot_param_zorders.ZORDER_PLOT_CELL_LABEL, fontsize=cell_font_size)
    ax.annotate(site_label, xy=(x,y+site_label_offset), alpha=alpha, zorder=plot_param_zorders.ZORDER_PLOT_CELL_LABEL, fontsize=site_font_size)
...
def interval_plot(dbcon, rat, df,  default_cell_color, detail_label_format, ax, map, force_ant_bw=None,
                  cell_arm_length=70, site_detail_label_format='',cellfile_layer=3, site_label_offset=0, custom_cell_color=None, site_font_size=None, cell_font_size=None):
    import azq_theme_manager

    dprint("azq_cell_file: interval_plot: df len {} columns {}".format(len(df), df.columns))
    ...
    for index, row in df.iterrows():
        ...
        dprint("azq_cell_file read row data: lat {}, lon {}, degree_plot {}, ant_bw {}, xy_pixel {}".format(lat, lon, degree_plot, ant_bw, xy_pixel))
        ...
        if detail_label_format == g_detail_label_format_dict[rat] or custom_cell_color is not None:
            if custom_cell_color is None:
                ...
                theme_df = azq_theme_manager.get_theme_df_for_column(custom_cell_color_db, dbcon=dbcon)
            ...
            if theme_df is not None:
                ...
                if "match_value" in theme_df.columns.values:
                    dprint("azq_cell_file - macth_value in cols - cell_match_value_color: {} col vals: {}".format(cell_match_value_color, theme_df.match_value.values))
                    if cell_match_value_color in theme_df.match_value.values:
                        cell_color = theme_df[ theme_df.match_value == cell_match_value_color ].ColorXml.values[0]
                        dprint("azq_cell_file - dyn gen theme: got maching color for cell:", cell_color)
                    else:
                        dprint("azq_cell_file - cell_match_value_color not in values")
                else:
                    print("WARNING: now we use 'match_value' col for xml pci match too - code should not reach here - check xml theme mode - search in Lower:")
                    if str(cell_match_value_color) in theme_df.Lower.values:
                        cell_color = theme_df[theme_df.Lower == str(cell_match_value_color)].ColorXml.values[0]
                        dprint("azq_cell_file - theme file: got matching color for cell:", cell_color)
                    else:
                        dprint("azq_cell_file - theme file: failed to get matching color for cell")
            else:
        ...
        clear_site_location()
        plot_cell(ax, xy_pixel[0], xy_pixel[1], degree_plot, ant_bw,
                  color=cell_color,
...
def read_cell_file(fp, extra_required_columns=[], raise_exception_if_check_failed=False):

    print("read_cell_file: raise_exception_if_check_failed:", raise_exception_if_check_failed)
    ...
    try:
        ...
        if raise_exception_if_check_failed:
            ...
            print("WARNING: raise_exception_if_check_failed False so dont raise - df_cellfile_check_and_convert() exception:", e)
    
    return df


g_cell_file_df_cache = {}
def get_cell_file_df(fp, extra_required_columns=[], raise_exception_if_check_failed=False):
    global g_cell_file_df_cache
    print("get_cell_file_df: raise_exception_if_check_failed:", raise_exception_if_check_failed) 
    ...
    df = read_cell_file(fp, extra_required_columns=extra_required_columns, raise_exception_if_check_failed=raise_exception_if_check_failed)
...
def apply_cell_file(dbcon, ax, map, map_bounds, force_ant_bw=None, cell_arm_length=70, cellfile_rat_list=["lte", "wcdma", "gsm"],
                    default_cell_color="#555555", cell_custom_label_format=None, site_detail_label_format='',sub_system=None ,cellfile_layer=3,
                    site_label_offset=0, custom_cell_color=None, site_font_size=None, cell_font_size=None):
    global g_cell_fp_dict
    global g_theme_dict
    global g_prev_plot_xy_check

    print(("apply_cell_file start cellfile_rat_list:", cellfile_rat_list, "g_cell_fp_dict:", g_cell_fp_dict))
    ...
    for rat in cellfile_rat_list:
        ...
        if g_cell_fp_dict[rat] is not None:
            ...
            interval_plot(dbcon, rat, df, default_cell_color, cell_label_format, ax, map, force_ant_bw, cell_arm_length=cell_arm_length, site_detail_label_format=site_detail_label_format,cellfile_layer=cellfile_layer, site_label_offset=site_label_offset, custom_cell_color=custom_cell_color, site_font_size=site_font_size, cell_font_size=cell_font_size)

            # only supported in LTE now
            if rat == "lte":
                # if custom cellfile was added inside phone log
                is_use_dynamic_cell_df, dynamic_lte_cell_df = get_dynamic_cell_info_flag(dbcon, rat)
                if is_use_dynamic_cell_df:
                    dprint("plotting is_use_dynamic_cell_df mode for rat:", rat)
                    df = dynamic_lte_cell_df
                    interval_plot(dbcon, rat, dynamic_lte_cell_df, default_cell_color, cell_label_format, ax, map, force_ant_bw, cell_arm_length=cell_arm_length,site_detail_label_format=site_detail_label_format,cellfile_layer=cellfile_layer, site_label_offset=site_label_offset, custom_cell_color=custom_cell_color, site_font_size=site_font_size, cell_font_size=cell_font_size)
...
def df_cellfile_check_and_convert(df, fp_for_error_report=None):
    ...
    for rename_dict in rename_dicts:
        ...
        for required_col in required_cols:
            ...
            if required_col not in df.columns:
                print("check required_col:", required_col, "not in df.columns:", df.columns)
        ...
        for required_col in required_cols:
            if required_col not in df.columns:
                raise Exception("required column: {} not present in cellfile at path: {}".format(required_col, fp_for_error_report))
    ...
    if len(df) == 0:
        raise Exception("Invalid: empty cellfile dataframe for cellfile at path: {}".format(fp_for_error_report))
