...
def get_chart_df(dbcon, param_list_dict, gc):
    ...
    for key in param_list_dict:
        ...
        if "selected_ue" in param_dict:
            selected_log = str(gc.log_list[int(param_dict["selected_ue"])-1])
            where = "where log_hash = '{}'".format(selected_log)
            param_alias_name = param_alias_name+"_UE_"+str(param_dict["selected_ue"])
...
def get_table_df_by_time(dbcon, time_before, param_list_dict, gc):
    ...
    for key in param_list_dict:
        ...
        if "selected_ue" in param_dict:
            selected_log = str(gc.log_list[int(param_dict["selected_ue"])-1])
            where = "and log_hash = '{}'".format(selected_log)
            param_alias_name = param_alias_name+"_UE_"+str(param_dict["selected_ue"])
