...
class custom_table_dialog(QtWidgets.QDialog):
    ...
    def __init__(self, gc, param_list=[]):
    ...
    def on_edit_parameter_button_click(self):
        dlg = add_custom_table_param_dialog.add_custom_table_param_dialog(self.on_param_edited, self.select_param, arg=self.select_arg)
        dlg.show()
    ...
    def merge_custom_param_to_df(self, param_list):
        ...
        if self.gc.databasePath is not None:
            with contextlib.closing(sqlite3.connect(self.gc.databasePath)) as dbcon:
                ...
                print("aaaaaaaaaaaaaaa",main_df, "bbbbbbbbbbbbb",df)
                for df in df_list:
    ...
    def on_ok_button_click(self):
        ...
        self.on_result.emit(df, self.param_list)