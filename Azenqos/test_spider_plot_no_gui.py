...
def test():
    ...
    df = spider_plot.gen_spider_df(cell_files, dbfp, "4G", "lte_physical_cell_id_1", single_point_layer_time=None)
    assert len(df) == 2
    assert abs(df.cell_lat.iloc[0] - 13.6695) < 0.000001
    assert abs(df.param_lat.iloc[0] - 13.668644) < 0.000001

    wkt_multiline_string, layer_name = spider_plot.gen_wkt_lines_plot_rat_spider(cell_files, dbfp, "4G", "lte_physical_cell_id_1")
