...
class module_dialog(QDialog):
    ...
    def __init__(self, parent, row_sr, gc, mdi):
        ...
        self.check = True
        self.row_sr = row_sr 
        self.param = "PY_EVAL "+self.row_sr["module"]+"."+self.row_sr["function_def"]
        self.setupUi()
    ...
    def setupUi(self):
        ...
        self.ui.param_te.setPlainText("PY_EVAL "+self.row_sr["module"]+"."+self.row_sr["function_def"])
        enable_slot = partial(self.enable_mod, self.ui.checkBox)
        disable_slot = partial(self.disable_mod, self.ui.checkBox)
        self.ui.checkBox.stateChanged.connect(lambda x: enable_slot() if x else disable_slot())
    ...
    def run_module(self): 
        ...
        window = datatable.create_table_window_from_api_expression_ret(None, title, self.gc, self.gc.login_dialog.server, self.gc.login_dialog.token, self.gc.login_dialog.lhl, mod_str)    
